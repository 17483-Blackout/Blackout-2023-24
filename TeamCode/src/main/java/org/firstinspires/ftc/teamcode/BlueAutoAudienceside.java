...
@Autonomous
public class BlueAutoAudienceside extends OpMode {
    ...
    @Override
    public void init() {
        Scalar lower = new Scalar(100, 50, 100); // the lower hsv threshold
        Scalar upper = new Scalar(190, 255, 255); // the upper hsv threshold
...
// now we can use recordedPropPosition to modify where we place the purple and yellow pixels
        switch (recordedPropPosition) {
            case LEFT:
                TrajectorySequence Left = drive.trajectorySequenceBuilder(new Pose2d(-35, 60, Math.toRadians(-90)))
...
// code to do if we saw the prop on the left
                break;
            case UNFOUND:
                TrajectorySequence Unfound = drive.trajectorySequenceBuilder(new Pose2d(-35, 60, Math.toRadians(-90)))
            ...
            case MIDDLE:
                TrajectorySequence Middle = drive.trajectorySequenceBuilder(new Pose2d(-35, 60, Math.toRadians(-90)))
                        .lineToConstantHeading(new Vector2d(-35,30))
                        .addTemporalMarker(1.5, () -> {
                            Servo3.setPosition(.9);
                        })
                ...
                // code to do if we saw team prop in the middle
                break;
            case RIGHT:
               TrajectorySequence Right = drive.trajectorySequenceBuilder(new Pose2d(-35, 60, Math.toRadians(-90)))
